...
export const addDepartment = async (req, res) => {
  try {
    ...
    const existingDepartment = await Department.findOne({ department: trimmedDepartment });
    ...
    // Fetch all existing department codes
    const departments = await Department.find({});
    let departmentCode;
    const codesInUse = new Set(departments.map(dep => dep.departmentCode));

    // Generate a unique department code
    for (let i = 1; ; i++) {
      const code = i < 10 ? `0${i}` : `${i}`; // Pad single digits
    ...
    const errors = { backendError: error.message || "An unexpected error occurred" };
    return res.status(500).json(errors);
  }
};
